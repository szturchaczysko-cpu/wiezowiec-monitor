...
with tab5:
    ...
    # === Pobierz case'y z result_pz="PZ6" ===
    @st.cache_data(ttl=60)
    def fetch_pz6_cases_for_date(date_str, prefixes_tuple):
        """Czyta z ew_cases + ew_cases_archived (per prefix) case'y z result_pz=PZ6,
        filtruje po completed_at w danej dacie (Europe/Warsaw), dedup po numer_zamowienia."""
        rows = []
        for prefix in prefixes_tuple:
            for coll_name in [f"{prefix}ew_cases", f"{prefix}ew_cases_archived"]:
                try:
                    # Filtr w Firestore: result_pz=PZ6. Filtr daty robimy w pamięci
                    # (Firestore wymagałby compound index na (result_pz, completed_at))
                    docs = db.collection(coll_name).where("result_pz", "==", "PZ6").stream()
                    for d in docs:
                        data = d.to_dict() or {}
                        completed_at = data.get("completed_at")
                        if not completed_at:
                            continue
                        # Konwertuj do daty w timezone PL
                        try:
                            if hasattr(completed_at, "astimezone"):
                                cd_str = completed_at.astimezone(tz_pl).strftime("%Y-%m-%d")
                            else:
                                cd_str = str(completed_at)[:10]
                        except Exception:
                            continue
                        if cd_str != date_str:
                            continue
    ...
    raw_rows = fetch_pz6_cases_for_date(t5_date_str, tuple(active_prefixes))
