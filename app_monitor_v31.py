...
with tab3:
    ...
    if not all_known:
        ...
        # Sort: najpierw ci z diamentami (po liczbie desc), potem reszta alfabetycznie
        ...
        picked = st.selectbox(
            "Operator:",
            options=all_known_sorted,
            key="_drilldown_op",
            format_func=lambda op: f"{op}  —  💎 {global_dia_t3.get(op, 0)}"
        )
        ...
        if picked:
            ...
            if dd_days_n < 1:
                ...
                per_day = {}     # date -> {"sessions": N, "diamonds": N, "transitions": {key: count}}
                period_total_dia = 0
                period_total_sess = 0
                period_total_cew = 0
                transitions_period = {}   # "PZ3_to_PZ6" -> count

                for d in dd_dates:
                    sdata = fetch_stats_for_date(d)
                    op_data = sdata.get(picked, {})
                    sess = op_data.get("sessions_completed", 0) if isinstance(op_data.get("sessions_completed"), (int, float)) else 0
                    dia = count_pz6_for_operator(op_data)

                    # Per-transition w tym dniu
                    day_trans = {}
                    ...
                    per_day[d] = {"sessions": sess, "diamonds": dia, "transitions": day_trans}
                    ...
                    per_day[d]["cases_ew"] = cew
                    period_total_cew += cew

                # METRYKI OKRESU
                st.markdown(f"#### Okres: {dd_from} – {dd_to} ({dd_days_n} dni)")
