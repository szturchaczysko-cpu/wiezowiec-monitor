...
# ============================================================
with tab4:
    st.header("📊 Prosty widok — diamenty (PZ6)")

    # === Pobierz dane ===
    typ_cache_dict4, cache_max_updated4, cache_n4 = fetch_typ_cache()
    cache_max_iso4 = cache_max_updated4.isoformat() if cache_max_updated4 else "none"
    all_cases_4 = fetch_pz6_cases_with_metadata(tuple(active_prefixes), cache_max_iso4)

    if not all_cases_4:
        st.info("Brak zarchiwizowanych case'ów w wybranym środowisku.")
        st.stop()

    # === Helper: czy case to PZ6 ===
    def _is_pz6(c):
        rp = c.get("result_pz")
        if rp is None:
            return False
        rp_str = str(rp).strip().upper()
        return rp_str in ("6", "PZ6", "PZ_6")

    # === Helper: wybierz datę aktywności case'a z fallback chain ===
    def _pick_case_date(c):
            ...
            if val:
                try:
                    if hasattr(val, "strftime"):
                        return val.strftime("%Y-%m-%d"), field
                    else:
                        s = str(val)[:10]
    ...
    # === Helper: kategoria znormalizowana do 3 grup ===
    def _kategoria_normalized(c):
        kat = c.get("towar_typ", "Brak danych")
        if kat == "Kolektor":
            return "Kolektor"
        if kat == "Skrzynia biegów":
            return "Skrzynia biegów"
        # Inne / Brak danych / Nieznany / null → Nieprzypisane
        return "Nieprzypisane"

    # Filtruj od razu do PZ6 + dorzuć display_date
    pz6_with_date = []
    date_source_counts = {"completed_at": 0, "started_at": 0, "assigned_at": 0, "archived_at": 0, "brak": 0}
    for c in all_cases_4:
        if not _is_pz6(c):
            continue
        date_str, date_src = _pick_case_date(c)
        if date_str:
            c2 = dict(c)
            c2["display_date"] = date_str
            c2["display_date_source"] = date_src
            pz6_with_date.append(c2)
            date_source_counts[date_src] = date_source_counts.get(date_src, 0) + 1
        else:
            date_source_counts["brak"] += 1

    if not pz6_with_date:
        st.warning("Brak diamentów (PZ6) z datą w wybranym środowisku.")
        with st.expander("🔍 Diagnostyka — co jest w danych"):
            unique_pz = sorted({str(c.get("result_pz")) for c in all_cases_4})
            st.write(f"**Łącznie case'ów:** {len(all_cases_4)}")
            st.write(f"**Unikalne `result_pz`:** {unique_pz}")
            st.write("Helper `_is_pz6` akceptuje: 6, '6', 'PZ6', 'PZ_6'.")
        st.stop()

    # Mały caption diagnostyczny o źródłach dat
    diag_bits = []
    for src in ("completed_at", "started_at", "assigned_at", "archived_at", "brak"):
        n = date_source_counts.get(src, 0)
        if n > 0:
            diag_bits.append(f"`{src}`: {n}")
    st.caption(
        f"💎 **{len(pz6_with_date)}** diamentów (PZ6) z datą. Źródła: " + " · ".join(diag_bits) +
        ". Fallback chain: completed → started → assigned → archived."
    )
    ...
    with fcol1:
        # Zakres dat — domyślnie cały zakres dostępny
        dates_in_data = sorted({c["display_date"] for c in pz6_with_date})
        min_d = datetime.strptime(dates_in_data[0], "%Y-%m-%d").date()
        max_d = datetime.strptime(dates_in_data[-1], "%Y-%m-%d").date()
    ...
    with fcol2:
        operators_in_data = sorted({c["operator"] for c in pz6_with_date if c.get("operator") and c["operator"] != "?"})
        operator_options = ["🌐 WSZYSCY OPERATORZY"] + operators_in_data
        picked_op = st.selectbox(
            "👤 Operator:",
            options=operator_options,
            index=0,
            key="_tab4_operator",
        )
        is_all_ops_4 = picked_op.startswith("🌐")
    ...
    filtered = []
    for c in pz6_with_date:
        if not (d_from_str <= c["display_date"] <= d_to_str):
            continue
        if not is_all_ops_4 and c["operator"] != picked_op:
            continue
        if typ_mode != "Wszystkie":
            if _kategoria_normalized(c) != TYP_FILTER_TARGET[typ_mode]:
                continue
        filtered.append(c)

    # === Metryki ===
    st.markdown("---")
    total_pz6 = len(filtered)
    avg_per_day = round(total_pz6 / days_in_range, 1) if days_in_range > 0 else 0
    unique_ops = len({c["operator"] for c in filtered})

    mcol1, mcol2, mcol3, mcol4 = st.columns(4)
    mcol1.metric("💎 PZ6 łącznie", total_pz6)
    mcol2.metric("📅 Dni w zakresie", days_in_range)
    mcol3.metric("📊 Średnio / dzień", avg_per_day)
    mcol4.metric("👥 Operatorów", unique_ops if is_all_ops_4 else 1)

    if total_pz6 == 0:
        st.warning(
            f"🔍 Brak diamentów dla wybranych filtrów "
            f"(zakres {d_from_str} → {d_to_str}, operator: {picked_op}, typ: {typ_mode})."
        )
        st.stop()
    ...
    st.markdown("---")
    if days_in_range > 1:
        st.markdown("### 📊 Diamenty per dzień")
        day_counts = {}
        cur = d_from
        while cur <= d_to:
        ...
        for c in filtered:
            ...
            if cd in day_counts:
                day_counts[cd] += 1
        df_day = pd.DataFrame(
            [{"Data": k, "PZ6": v} for k, v in day_counts.items()]
        )
        st.bar_chart(df_day.set_index("Data"), height=420)

        # Linia trendu pod spodem
        st.markdown("### 📈 Trend dzienny")
        st.line_chart(df_day.set_index("Data"), height=280)
    else:
        # Zakres = 1 dzień
        if is_all_ops_4:
            st.markdown(f"### 📊 Diamenty per operator — {d_from_str}")
            op_counts = {}
            for c in filtered:
                op_counts[c["operator"]] = op_counts.get(c["operator"], 0) + 1
            df_op = pd.DataFrame(
                [{"Operator": k, "PZ6": v} for k, v in op_counts.items()]
            ).sort_values("PZ6", ascending=False)
            st.bar_chart(df_op.set_index("Operator"), height=420)
        else:
            st.success(f"💎 **{total_pz6}** diamentów dla **{picked_op}** w dniu **{d_from_str}**.")

    # === Drugi wykres: per operator (gdy WSZYSCY i zakres > 1 dzień) ===
    if is_all_ops_4 and days_in_range > 1:
        st.markdown("---")
        st.markdown("### 📊 Suma per operator w zakresie")
        op_counts = {}
        for c in filtered:
            op_counts[c["operator"]] = op_counts.get(c["operator"], 0) + 1
        df_op = pd.DataFrame(
    ...
    # === Rozkład typu — tylko jak filtr typu = Wszystkie ===
    if typ_mode == "Wszystkie":
        st.markdown("---")
        st.markdown("### 🏷️ Rozkład per typ towaru")
        typ_counts = {"Kolektor": 0, "Skrzynia biegów": 0, "Nieprzypisane": 0}
        for c in filtered:
            typ_counts[_kategoria_normalized(c)] += 1
        df_typ = pd.DataFrame(
            [{"Typ towaru": k, "PZ6": v} for k, v in typ_counts.items()]
        )
        tcol1, tcol2 = st.columns([2, 1])
        with tcol1:
            st.bar_chart(df_typ.set_index("Typ towaru"), height=300)
        with tcol2:
            st.markdown("**Wartości:**")
            for _, row in df_typ.iterrows():
                pct = round(row["PZ6"] / total_pz6 * 100, 1) if total_pz6 else 0
                st.markdown(f"- **{row['Typ towaru']}**: {row['PZ6']} ({pct}%)")

    # === Podsumowanie tekstowe ===
    st.markdown("---")
    st.caption(
        f"📋 **{total_pz6}** diamentów (PZ6) | typ: {typ_mode} | operator: {picked_op} | "
        f"zakres: {d_from_str} → {d_to_str} ({days_in_range} dni). "
        f"Źródło: `{', '.join(f'{p}ew_cases_archived' for p in active_prefixes)}`."
    )
