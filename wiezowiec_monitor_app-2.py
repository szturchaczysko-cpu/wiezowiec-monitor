...
for d in dates:
    ...
    for doc in docs:
        ...
        diamonds = data.get("diamonds_today", 0)
        day_total += completed
        day_diamonds += diamonds
        op = doc.id
        if op not in op_daily:
            op_daily[op] = {}
            op_daily_diamonds[op] = {}
        op_daily[op][d] = completed
        op_daily_diamonds[op][d] = diamonds
    daily_totals[d] = day_total
    daily_diamonds[d] = day_diamonds

# Metryki okresu
period_cases = sum(daily_totals.values())
period_diamonds = sum(daily_diamonds.values())
col_ew1, col_ew2, col_ew3 = st.columns(3)
col_ew1.metric("🏢 Casy EW (okres)", period_cases)
col_ew2.metric("💎 Diamenty (okres)", period_diamonds)
col_ew3.metric("📅 Dni", len(dates))

if any(v > 0 for v in daily_totals.values()):
    # Wykres dzienny — casy + diamenty
    df_daily = pd.DataFrame({
        "Data": list(daily_totals.keys()),
        "Zakończone casy": list(daily_totals.values()),
        "💎 Diamenty": list(daily_diamonds.values()),
    }).sort_values("Data")
    st.area_chart(df_daily.set_index("Data"))

    # Tabela per operator per dzień (casy + diamenty)
    if op_daily:
        st.subheader("📊 Casy per operator per dzień")
        rows = []
        for op, day_map in op_daily.items():
            row = {"Operator": op}
            row["Casy"] = sum(day_map.values())
            row["💎"] = sum(op_daily_diamonds.get(op, {}).values())
            for d in sorted(dates):
                row[d] = day_map.get(d, 0)
            rows.append(row)
        df_opd = pd.DataFrame(rows).sort_values(by="Casy", ascending=False)
