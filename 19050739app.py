    ...
    if not all_known:
        ...
        if picked:
            ...
            if dd_days_n < 1:
                ...
                # --- PROMPTY DLA AGENTA ---
                with st.expander("🤖 Odśwież cache w agencie (skopiuj prompt → wklej w Dashboard DSG)", expanded=(n_no_data > 0 and cache_n == 0)):
                    st.markdown(
                        "**Jak to działa:** monitor sam nie ma dostępu do baz SQL, dlatego cache typów buduje agent. "
                        "Skopiuj poniższy prompt, wklej w Dashboard DSG → agent diagnostyczny, poczekaj na zakończenie. "
                        "Potem wróć tutaj i kliknij **🔄 Pobierz świeży cache**."
                    )

                    refresh_mode = st.radio(
                        "Tryb odświeżania:",
                        options=[
                            f"Tylko brakujące ({n_no_data} case'ów)",
                            "Pełny refresh wszystkich case'ów",
                        ],
                        index=0 if n_no_data > 0 else 1,
                        key="_refresh_mode",
                        horizontal=True,
                    )

                    if refresh_mode.startswith("Tylko"):
                        # Zbuduj listę brakujących numerów — filtr: czysto numeryczne 5–7 znaków
                        def _is_valid_nr(s):
                            s = str(s or "").strip()
                        ...
                        if skipped_garbage:
                            st.caption(f"⚠️ Odfiltrowano {skipped_garbage} case'ów z niepoprawnym numerem zamówienia (puste / 'i, wyżej' / 'ZW...' / inne literałki z CSV).")

                        if not missing_nrs:
                            st.success("✅ Brak case'ów do uzupełnienia — wszystkie mają już typ z cache lub regexa.")
                            agent_prompt = ""
                        else:
                            # === PODZIAŁ NA PACZKI ===
                            st.markdown("**📦 Rozmiar paczki** (ile numerów na jeden bieg agenta):")
                            PACK_SIZE = st.radio(
                                "Rozmiar paczki:",
                                ...
                                help="Mniejsza paczka = więcej biegów ale każdy bieg bezpieczniejszy (mniej iteracji agenta = mniejsza szansa na 'KeyError' platformy DSG). Default 400."
                            )
                            packs = [missing_nrs[i:i+PACK_SIZE] for i in range(0, len(missing_nrs), PACK_SIZE)]
                            n_packs = len(packs)
                            est_batches_per_pack = (PACK_SIZE + 79) // 80  # ile batchy MSSQL per paczka
                            est_iter_per_pack = est_batches_per_pack + 3   # + PG + JSON + bufor
                            st.caption(
                                f"📊 {len(missing_nrs)} numerów → **{n_packs} paczek** po ~{PACK_SIZE} numerów. "
                                f"Każda paczka ~{est_batches_per_pack} SQL MSSQL + 1 PG + JSON = **~{est_iter_per_pack} iteracji**."
                            )

                            # Status paczek z cache
                            pack_status_labels = []
                            for i, p in enumerate(packs):
                                in_cache = sum(1 for nr in p if nr in typ_cache_dict)
                                pct = in_cache / len(p) * 100 if p else 0
                                first, last = p[0], p[-1]
                                if pct >= 80:
                                    emoji = "✅"
                                    state = f"zaimportowana ({in_cache}/{len(p)})"
                                elif pct >= 20:
                                    emoji = "🟡"
                                    state = f"częściowa ({in_cache}/{len(p)})"
                                else:
                                    emoji = "⏳"
                                    state = f"czeka ({in_cache}/{len(p)})"
                                pack_status_labels.append(f"{emoji} Paczka {i+1}/{n_packs} — {state} | {first}–{last}")

                            # Auto-podpowiedź: pierwsza paczka która nie jest zaimportowana
                            default_pack = 0
                            for i, p in enumerate(packs):
                                in_cache = sum(1 for nr in p if nr in typ_cache_dict)
                                if (in_cache / len(p) if p else 0) < 0.8:
                                    default_pack = i
                                    break

                            picked_idx = st.selectbox(
                                "Wybierz paczkę do przygotowania:",
                                options=list(range(n_packs)),
                                format_func=lambda i: pack_status_labels[i],
                                index=default_pack,
                                key="_pack_picker",
                            )
                            picked_pack = packs[picked_idx]

                            # === BATCHE SQL DLA WYBRANEJ PACZKI ===
                            BATCH_SIZE = 80
                            batches = [picked_pack[i:i+BATCH_SIZE] for i in range(0, len(picked_pack), BATCH_SIZE)]
                            n_batches = len(batches)
                            sql_blocks = []
                            for i, b in enumerate(batches, 1):
                                sql_blocks.append(
                                    f"SQL {i}/{n_batches}: WITH r AS (SELECT zksZknZamNr, zksIndex_WEB, ROW_NUMBER() OVER (PARTITION BY zksZknZamNr ORDER BY zksCenaNetto DESC) rn FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr IN ({', '.join(b)}) AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '') SELECT STRING_AGG(CAST(zksZknZamNr AS varchar) + '=' + zksIndex_WEB, '|') AS p FROM r WHERE rn = 1"
                                )
                            sqls_inline = "\n\n".join(sql_blocks)

                            agent_prompt = f"""WAŻNE: ustaw w UI **Max iter ≥ {n_batches + 5}** i **Max SQL ≥ {n_batches + 2}** przed startem.

To jest **paczka {picked_idx + 1} z {n_packs}** ({len(picked_pack)} numerów: {picked_pack[0]}–{picked_pack[-1]}).

Wykonaj DOKŁADNIE poniższe kroki. Nie komentuj między SQL-ami. NIE używaj save_finding. NIE testuj składni. NIE optymalizuj. Wykonuj po kolei.

═══════════ KROK 1: {n_batches} zapytań do MSSQL ═══════════
Wykonaj kolejno {n_batches} SQL-i poniżej, każdy zwróci ~80 par w polu `p`. Sklej wszystkie pary do jednego mapowania `nr→indeks` w pamięci.

{sqls_inline}

═══════════ KROK 2: 1 zapytanie do Postgres ═══════════
Z kroku 1 weź `set(values())` — unikalne indeksy. Wykonaj JEDEN SQL:

SELECT a."artIndeks", ta."tartID", ta."tartNazwa" FROM "public"."wmsArtykuly" a JOIN "public"."wmsTypArtykulu" ta ON ta."tartID" = a."artTyp" WHERE a."artIndeks" IN (<unikalne indeksy z kroku 1>)

═══════════ KROK 3: zmapuj kategorie ═══════════
W PAMIĘCI (bez SQL): dla każdego (numer, indeks) z kroku 1:
- znajdź (tartID, tartNazwa) dla indeksu z kroku 2 (jeśli brak → kategoria='Nieznany', tartID=null, tartNazwa=null, source='lookup_failed')
- tartID ∈ (4, 10, 15) → 'Kolektor'
- tartID ∈ (3, 5, 6, 7, 9, 13, 14, 16) → 'Skrzynia biegów'
- reszta → 'Inne'
- source = 'sql'

═══════════ KROK 4: zwróć JSON ═══════════
NA KOŃCU odpowiedzi, w bloku ```json (BEZ KOMENTARZY):

```json
{{
  "360603": {{"kategoria": "Kolektor", "tartNazwa": "Kolektor", "tartID": 4, "resolved_index": "ORG30_BMW_SAKS_PRZE_KPL", "source": "sql"}},
  "362815": {{"kategoria": "Skrzynia biegów", "tartNazwa": "Skrzynia", "tartID": 3, "resolved_index": "20620MBGRUP10", "source": "sql"}}
}}
```

Plus krótkie podsumowanie: ile Kolektor / Skrzynia / Inne / Nieznany w **tej paczce**.

PACZKA: {picked_idx + 1}/{n_packs} | NUMERÓW: {len(picked_pack)} | BATCHY MSSQL: {n_batches} | OCZEKIWANE TRANSAKCJE: {n_batches + 1} SQL + 1 odpowiedź JSON."""
                    else:
                        agent_prompt = """Zbuduj PEŁNE mapowanie numer_zamowienia → typ towaru dla WSZYSTKICH case'ów w Firebase i zwróć jako JSON.

KROK A: Pobierz z Firebase wszystkie unikalne numer_zamowienia z:
- `test_ew_cases_archived`
- `test_ew_cases`
- `ew_cases_archived` (jeśli masz read)
- `ew_cases`

Filtr: tylko czysto NUMERYCZNE wartości 5–7 cyfr.

KROK B: Pobierz mapowanie z SQL hurtowo:
1. MSSQL STEEPC: SELECT zksZknZamNr, zksIndex_WEB FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr IN (<wszystkie>) AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '' ORDER BY zksZknZamNr, zksCenaNetto DESC. Weź pierwszy indeks per numer.
2. Postgres maggo: SELECT a."artIndeks", ta."tartID", ta."tartNazwa" FROM "public"."wmsArtykuly" a JOIN "public"."wmsTypArtykulu" ta ON ta."tartID" = a."artTyp" WHERE a."artIndeks" IN (<wszystkie indeksy z kroku 1 + niepuste index_handlowy z Firebase>).

KROK C: Mapuj kategorię:
- tartID ∈ (4, 10, 15) → 'Kolektor'
- tartID ∈ (3, 5, 6, 7, 9, 13, 14, 16) → 'Skrzynia biegów'
- reszta → 'Inne'

KROK D: Zwróć JSON w bloku ```json — klucz: numer_zamowienia (string), wartość: dict {kategoria, tartNazwa, tartID, resolved_index, source}.

Plus podsumowanie tekstowe."""

                    if agent_prompt:
                        st.code(agent_prompt, language="text")
                        st.caption(f"📋 Zaznacz cały tekst powyżej (Ctrl+A wewnątrz bloku albo kliknij ikonkę kopiowania w prawym górnym rogu kodu) i wklej do agenta.")

                    # === IMPORT WYNIKU AGENTA ===
                    st.markdown("---")
                    st.markdown("**📥 Wklej wynik agenta (JSON):**")
                    st.caption("Agent zwróci blok ```json {...}```. Skopiuj jego ZAWARTOŚĆ (sam JSON, bez backticków `​`​`​) i wklej poniżej. Monitor zapisze wpisy do Firestore.")
                    json_input = st.text_area("JSON:", height=180, key="_agent_json_input", placeholder='{"360603": {"kategoria": "Kolektor", ...}, ...}')
                    col_imp1, col_imp2 = st.columns([1, 4])
                    with col_imp1:
                        do_import = st.button("💾 Zapisz do cache", type="primary", key="_btn_import_cache")
                    if do_import:
                        if not json_input.strip():
                            st.error("Pusty JSON.")
                        else:
                            # Spróbuj wyciągnąć JSON z bloku ```json``` jeśli user wkleił z markdownem
                            raw = json_input.strip()
                            if raw.startswith("```"):
                                # usuń backticki + opcjonalny prefix języka
                                lines = raw.split("\n")
                            ...
                            try:
                                ...
                                if not isinstance(parsed, dict):
                                    st.error(f"JSON musi być obiektem (mapping numer→dane), dostałem: {type(parsed).__name__}")
                                else:
                                    n_saved = 0
                                    n_skipped = 0
                                    errors = []
                                    now_ts = datetime.now(tz_pl)
                                    for nr, payload in parsed.items():
                                        try:
                                            ...
                                            if not nr_str:
                                                ...
                                                continue
                                            doc_data = {
                                                "numer_zamowienia": nr_str,
                                                "resolved_index": payload.get("resolved_index"),
                                                "tartID": payload.get("tartID"),
                                                "tartNazwa": payload.get("tartNazwa"),
                                                "kategoria": payload.get("kategoria", "Nieznany"),
                                                "source": payload.get("source", "agent"),
                                                "updated_at": now_ts,
                                            }
                                            db.collection("typ_towaru_cache").document(nr_str).set(doc_data, merge=True)
                                            n_saved += 1
                                        except Exception as e:
                                            errors.append(f"{nr}: {e}")
                                            n_skipped += 1
                                    st.success(f"✅ Zapisano {n_saved} wpisów do `typ_towaru_cache`. Pominięto: {n_skipped}.")
                                    if errors:
                                        with st.expander(f"⚠️ {len(errors)} błędów"):
                                            for e in errors[:20]:
                                                st.text(e)
                                    st.cache_data.clear()
                                    st.info("🔄 Cache odświeżony. Przewiń w górę aby zobaczyć zaktualizowane dane.")
                            except json.JSONDecodeError as e:
                                st.error(f"❌ Błąd parsowania JSON: {e}")
                                st.code(raw[:500], language="text")

                # --- Alert gdy wszystko z cache jest puste ---
                if cache_n == 0:
                    st.warning(
                        "⚠️ Kolekcja `typ_towaru_cache` jest pusta. "
                        "Otwórz powyższy expander, skopiuj prompt **Pełny refresh** i wklej do agenta — "
                        "po jego biegu wróć tutaj i kliknij 🔄 Pobierz świeży cache."
                    )
