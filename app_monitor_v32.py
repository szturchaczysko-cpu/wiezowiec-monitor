...
def fetch_pz6_cases_with_metadata(_prefixes_tuple):
    """Pobiera wszystkie case'y zakończone PZ6 z aktywnych prefixów + parsuje typ towaru.
    _prefixes_tuple służy tylko do invalidacji cache przy zmianie środowiska."""
    prefixes = list(_prefixes_tuple)
    results = []
    seen = set()  # (collection, case_id) — dedup
    for prefix in prefixes:
        for col_name in (f"{prefix}ew_cases_archived", f"{prefix}ew_cases"):
            try:
                docs = list(db.collection(col_name).where("result_pz", "==", "PZ6").get())
            except Exception:
                continue
            for d in docs:
                key = (col_name, d.id)
                ...
                # completed_at może być datetime lub None
                completed_date = None
                if completed_at:
                    try:
                ...
                results.append({
                    ...
                    "towar_typ_raw": parse_towar_typ(data),
                    "towar_typ": normalize_typ(parse_towar_typ(data)),
    ...
    if not all_known:
        ...
        if picked:
            ...
            if dd_days_n < 1:
                ...
                st.caption(
                    "Źródło: zakończone case'y z `ew_cases_archived` (i żywych `ew_cases`) gdzie `result_pz=PZ6`. "
                    "Typ parsowany regexem z `result_tag` → `autopilot_messages[-1]` → `pelna_linia_szturchacza`. "
                    "Case'y bez literału w żadnym z pól lecą jako **Nieznany**."
                )

                pz6_all = fetch_pz6_cases_with_metadata(tuple(active_prefixes))
                ...
                if not pz6_filtered:
                    st.info(
                        f"Brak zakończonych case'ów PZ6 dla "
                        f"{'wszystkich operatorów' if is_all_ops else picked} "
                        f"w aktywnych kolekcjach. "
                        f"(W trybie test_ archiwum może być prawie puste — przełącz na 'prod' lub sumę w sidebarze.)"
                    )
                else:
                    # Podsumowanie globalne dla wybranego zakresu (wszystkie PZ6 dla operatora/wszystkich)
                    typ_counts_all = {}
                    for c in pz6_filtered:
                    ...
                    n_unk = typ_counts_all.get("Nieznany", 0)
                    colT_total.metric("📦 Razem PZ6 (cała historia)", total_pz6)
                    colT_kol.metric(
                        "🔧 Kolektor",
                        n_kol,
                    ...
                    colT_unk.metric(
                        "❓ Nieznany",
                        n_unk,
                        f"{round(n_unk / total_pz6 * 100, 1)}%" if total_pz6 > 0 else "0%"
                    )
                    ...
                    # Jeśli WSZYSCY — tabela operator × typ
                    if is_all_ops:
                    ...
                    # Dzienne diamenty per typ w zakresie dat
                    if pz6_in_range:
                        st.markdown(f"**📈 W okresie {dd_from} – {dd_to}: {len(pz6_in_range)} PZ6 z datą zakończenia**")
                    ...
                    if pz6_no_date:
                        st.caption(f"ℹ️ {len(pz6_no_date)} case'ów bez `completed_at` — nie wpadły do dziennego wykresu, ale są w sumarycznych liczbach powyżej.")

                    # Tabela szczegółowa case'ów (zwijana)
                    with st.expander(f"📋 Pełna lista zakończonych PZ6 ({len(pz6_filtered)} case'ów)"):
                        detail_rows = []
                        for c in pz6_filtered:
                            detail_rows.append({
                                "Data": c.get("completed_date") or "?",
                                "Operator": c.get("operator", "?"),
                                "Grupa": c.get("grupa", "?"),
                                "Nr zamówienia": c.get("numer_zamowienia", "?"),
                                "Typ": c.get("towar_typ", "Nieznany"),
                                "Kurier": c.get("kurier", "?"),
                                "Case ID": c.get("case_id", "?"),
                            })
                        df_detail2 = pd.DataFrame(detail_rows).sort_values(by="Data", ascending=False)
