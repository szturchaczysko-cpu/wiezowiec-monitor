...
# ============================================================
with tab4:
    st.header("📊 Prosty widok — diamenty per zakres dat")
    ...
    # Tylko diamenty (PZ6)
    pz6_all_4 = [c for c in all_cases_4 if c.get("result_pz") == 6]

    if not pz6_all_4:
        st.info("Brak zarchiwizowanych diamentów (PZ6) w wybranym środowisku.")
        st.stop()
    ...
    with fcol1:
        ...
        dates_in_data = sorted({c["completed_date"] for c in pz6_all_4 if c.get("completed_date")})
        if dates_in_data:
    ...
    with fcol2:
        # Wybór operatora
        operators_in_data = sorted({c["operator"] for c in pz6_all_4 if c.get("operator") and c["operator"] != "?"})
    ...
    pz6_filtered_4 = [
        c for c in pz6_all_4
        if c.get("completed_date")
        and d_from_str <= c["completed_date"] <= d_to_str
        and (is_all_ops_4 or c["operator"] == picked_op)
    ]

    if not pz6_filtered_4:
        st.warning(f"🔍 Brak diamentów w zakresie **{d_from_str} – {d_to_str}** dla **{picked_op}**.")
        st.stop()

    # === Metryki ===
    st.markdown("---")
    total_diamonds = len(pz6_filtered_4)
    days_in_range = (d_to - d_from).days + 1
    avg_per_day = round(total_diamonds / days_in_range, 1) if days_in_range > 0 else 0
    unique_ops = len({c["operator"] for c in pz6_filtered_4})

    mcol1, mcol2, mcol3, mcol4 = st.columns(4)
    mcol1.metric("💎 Diamentów łącznie", total_diamonds)
    ...
    if is_all_ops_4:
        ...
        st.markdown(f"### 📊 Diamenty per dzień — {picked_op}")
        day_counts = {}
        cur = d_from
        while cur <= d_to:
        ...
        for c in pz6_filtered_4:
            ...
            if cd in day_counts:
                day_counts[cd] += 1
        df_day = pd.DataFrame(
            [{"Data": k, "Diamenty": v} for k, v in day_counts.items()]
        )
    ...
    df_trend = pd.DataFrame(
        [{"Data": k, "Diamenty": v} for k, v in day_counts_all.items()]
    )
    ...
    df_typ = pd.DataFrame(
        [{"Typ towaru": k, "Diamenty": v} for k, v in typ_counts.items()]
    ).sort_values("Diamenty", ascending=False)
    ...
    with tcol2:
        ...
        for _, row in df_typ.iterrows():
            pct = round(row["Diamenty"] / total_diamonds * 100, 1) if total_diamonds else 0
            st.markdown(f"- **{row['Typ towaru']}**: {row['Diamenty']} ({pct}%)")

    # === Podsumowanie tekstowe ===
    st.markdown("---")
    st.caption(
        f"📋 Pokazuję **{total_diamonds}** diamentów (PZ6) z zakresu **{d_from_str} → {d_to_str}** "
