                ...
                with st.expander("🤖 Odśwież cache w agencie (skopiuj prompt → wklej w Dashboard DSG)", expanded=(n_no_data > 0 and cache_n == 0)):
                    ...
                    if refresh_mode.startswith("Tylko"):
                        ...
                        if not missing_nrs:
                            ...
                            # Pociąć listę na batche po 80 numerów (limit transportu STRING_AGG ≈ 90)
                            BATCH_SIZE = 80
                            batches = [missing_nrs[i:i+BATCH_SIZE] for i in range(0, len(missing_nrs), BATCH_SIZE)]
...
Z kroku 1 weź `set(values())` — unikalne indeksy (~200-300 sztuk). Wykonaj JEDEN SQL:
...
Plus krótkie podsumowanie: ile Kolektor / Skrzynia / Inne / Nieznany.

LICZBA NUMERÓW DO PRZETWORZENIA: {len(missing_nrs)} | LICZBA BATCHY MSSQL: {n_batches} | OCZEKIWANE TRANSAKCJE: {n_batches + 1} SQL + 1 odpowiedź JSON."""
                    else:
