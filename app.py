...
# ============================================================
with tab4:
    st.header("📊 Prosty widok — zarchiwizowane case'y per zakres dat")
    ...
    with fcol1:
        ...
        dates_in_data = sorted({c["completed_date"] for c in all_cases_4 if c.get("completed_date")})
        if dates_in_data:
            min_d = datetime.strptime(dates_in_data[0], "%Y-%m-%d").date()
            max_d = datetime.strptime(dates_in_data[-1], "%Y-%m-%d").date()
        else:
            today_pl = datetime.now(tz_pl).date()
            min_d = today_pl - timedelta(days=90)
            max_d = today_pl

        default_from = max(min_d, max_d - timedelta(days=30))
        date_range = st.date_input(
            "📅 Zakres dat (data zakończenia case'a):",
        ...
        if isinstance(date_range, tuple) and len(date_range) == 2:
            ...
            d_from, d_to = default_from, max_d

    with fcol2:
        # Wybór operatora
        operators_in_data = sorted({c["operator"] for c in all_cases_4 if c.get("operator") and c["operator"] != "?"})
    ...
    with fcol3:
        ...
        pz_mode = st.radio(
            "🔢 Pokaż:",
            options=["Wszystkie", "Tylko PZ6 💎", "Bez PZ6"],
            index=0,
            key="_tab4_pz_filter",
        )
    ...
    with fcol4:
        ...
        typ_mode = st.radio(
            "🏷️ Typ towaru:",
            options=["Wszystkie", "🔩 Kolektor", "🔧 Skrzynia biegów", "📦 Inne", "❓ Brak danych"],
            index=0,
            key="_tab4_typ_filter",
        )

    # === Helper: czy case to PZ6 (obsługa różnych typów wartości) ===
    def _is_pz6(c):
        rp = c.get("result_pz")
        if rp is None:
            return False
    ...
    # === Mapowanie filtra typu → nazwa kategorii w danych ===
    TYP_FILTER_MAP = {
        "🔩 Kolektor": "Kolektor",
    ...
    for c in all_cases_4:
        cd = c.get("completed_date")
        if not cd or not (d_from_str <= cd <= d_to_str):
            continue
        if not is_all_ops_4 and c["operator"] != picked_op:
            continue
        if pz_mode == "Tylko PZ6 💎" and not _is_pz6(c):
            continue
        if pz_mode == "Bez PZ6" and _is_pz6(c):
            continue
        if typ_mode != "Wszystkie":
            wanted_kat = TYP_FILTER_MAP.get(typ_mode)
            case_kat = c.get("towar_typ", "Brak danych")
            # 'Nieznany' z cache traktuj jak 'Brak danych' dla użytkownika
            if case_kat == "Nieznany":
                case_kat = "Brak danych"
            if case_kat != wanted_kat:
                continue
        pz6_filtered_4.append(c)

    if not pz6_filtered_4:
        st.warning(
            f"🔍 Brak case'ów w zakresie **{d_from_str} – {d_to_str}** "
            f"dla **{picked_op}** (PZ: {pz_mode}, Typ: {typ_mode})."
        )
        # Pokaż statystyki diagnostyczne
        with st.expander("🔍 Diagnostyka — co jest w danych"):
            sample = all_cases_4[:5]
            st.write("**Próbka 5 case'ów (surowe pola):**")
            for c in sample:
                st.json({
                    "operator": c.get("operator"),
                    "completed_date": c.get("completed_date"),
                    "result_pz": c.get("result_pz"),
                    "status": c.get("status"),
                })
            unique_pz = sorted({str(c.get("result_pz")) for c in all_cases_4})
            st.write(f"**Unikalne wartości `result_pz` w danych:** {unique_pz}")
            unique_dates = sorted({c.get("completed_date") for c in all_cases_4 if c.get("completed_date")})
            if unique_dates:
                st.write(f"**Zakres dat w danych:** {unique_dates[0]} → {unique_dates[-1]} ({len(unique_dates)} unikalnych dat)")
            else:
                st.write("**Brak dat (completed_date)** w żadnym case'ie.")
        st.stop()

    # === Metryki ===
    st.markdown("---")
    total_cases = len(pz6_filtered_4)
    days_in_range = (d_to - d_from).days + 1
    avg_per_day = round(total_cases / days_in_range, 1) if days_in_range > 0 else 0
    unique_ops = len({c["operator"] for c in pz6_filtered_4})
    n_pz6 = sum(1 for c in pz6_filtered_4 if _is_pz6(c))

    mcol1, mcol2, mcol3, mcol4, mcol5 = st.columns(5)
    mcol1.metric("📋 Case'ów łącznie", total_cases)
    mcol2.metric("💎 Z tego PZ6", n_pz6)
    mcol3.metric("📅 Dni w zakresie", days_in_range)
    mcol4.metric("📊 Średnio / dzień", avg_per_day)
    mcol5.metric("👥 Operatorów", unique_ops if is_all_ops_4 else 1)

    # === Wykres 1: SŁUPKI ===
    st.markdown("---")
    if is_all_ops_4:
        st.markdown("### 📊 Case'y per operator")
        op_counts = {}
        for c in pz6_filtered_4:
            op_counts[c["operator"]] = op_counts.get(c["operator"], 0) + 1
        df_op = pd.DataFrame(
            [{"Operator": k, "Cases": v} for k, v in op_counts.items()]
        ).sort_values("Cases", ascending=False)
        st.bar_chart(df_op.set_index("Operator"), height=420)
    else:
        st.markdown(f"### 📊 Case'y per dzień — {picked_op}")
        day_counts = {}
        cur = d_from
        while cur <= d_to:
        ...
        for c in pz6_filtered_4:
            ...
            if cd in day_counts:
                day_counts[cd] += 1
        df_day = pd.DataFrame(
            [{"Data": k, "Cases": v} for k, v in day_counts.items()]
        )
        st.bar_chart(df_day.set_index("Data"), height=420)

    # === Wykres 2: TREND CZASOWY (liniowy) ===
    st.markdown("---")
    st.markdown("### 📈 Trend dzienny (cały zakres)")
    day_counts_all = {}
    cur = d_from
    while cur <= d_to:
        day_counts_all[cur.strftime("%Y-%m-%d")] = 0
        cur += timedelta(days=1)
    for c in pz6_filtered_4:
        cd = c["completed_date"]
        if cd in day_counts_all:
            day_counts_all[cd] += 1
    df_trend = pd.DataFrame(
        [{"Data": k, "Cases": v} for k, v in day_counts_all.items()]
    )
    st.line_chart(df_trend.set_index("Data"), height=320)

    # === Wykres 3: ROZKŁAD PER TYP TOWARU ===
    st.markdown("---")
    st.markdown("### 🏷️ Rozkład per typ towaru")
    typ_counts = {}
    for c in pz6_filtered_4:
        t = c.get("towar_typ", "Brak danych")
        typ_counts[t] = typ_counts.get(t, 0) + 1
    df_typ = pd.DataFrame(
        [{"Typ towaru": k, "Cases": v} for k, v in typ_counts.items()]
    ).sort_values("Cases", ascending=False)
    tcol1, tcol2 = st.columns([2, 1])
    with tcol1:
        st.bar_chart(df_typ.set_index("Typ towaru"), height=320)
    with tcol2:
        st.markdown("**Wartości:**")
        for _, row in df_typ.iterrows():
            pct = round(row["Cases"] / total_cases * 100, 1) if total_cases else 0
            st.markdown(f"- **{row['Typ towaru']}**: {row['Cases']} ({pct}%)")

    # === Podsumowanie tekstowe ===
    st.markdown("---")
    st.caption(
        f"📋 Pokazuję **{total_cases}** case'ów (PZ: {pz_mode}, Typ: {typ_mode}) "
        f"z zakresu **{d_from_str} → {d_to_str}** "
        f"({days_in_range} dni) dla **{picked_op}**. "
        f"Źródło: zarchiwizowane case'y w `{', '.join(f'{p}ew_cases_archived' for p in active_prefixes)}`."
    )
