...
def count_pz6_for_operator(stats_data):
    ...
    UWAGA: ten licznik jest NIEDOKŁADNY (zlicza transitions, nie unikalne case'y).
    Dokładne liczby są w zakładce "🎯 Diamenty dokładne" która czyta result_pz=PZ6 z ew_cases.
    """
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    ...
    "📊 Prosty widok",
    "🎯 Diamenty dokładne (ew_cases)",
    "💎 Diamenty (forum / ew_diamond_log)",
])
...
# ============================================================
# ============================================================
# ============== ZAKŁADKA 5: DIAMENTY DOKŁADNE (ew_cases.result_pz)
# ============================================================
with tab5:
    st.header("🎯 Diamenty dokładne — z `ew_cases.result_pz`")
    st.caption(
        "Źródło prawdy: pole `result_pz=PZ6` w samym dokumencie case'a (`ew_cases` + `ew_cases_archived`). "
        "Każde kliknięcie 'Zakończ → Następny' z PZ6 ustawia/nadpisuje to pole. **1 case = 1 dokument = 1 diament** "
        "niezależnie ile razy operator klikał. AutoSzturchacz (chatbot forum) NIE jest tu — pokazuje go diff z forum."
    )
    ...
    # === Pobierz case'y z result_pz=PZ6 (z diagnostyką) ===
    @st.cache_data(ttl=60)
    def fetch_pz6_cases_for_date(date_str, prefixes_tuple):
        """Czyta z ew_cases + ew_cases_archived (per prefix) case'y zakończone na PZ6.
        Toleruje różne warianty wartości result_pz (PZ6, pz6, 6, "6", PZ_6)."""
        rows = []
        ...
        diag = {
            ...
            "with_result_pz": 0,
            "with_completed_at": 0,
            "result_pz_values": Counter(),
            "completed_at_dates_yyyy_mm_dd": Counter(),
            "by_collection": {},
        }
        PZ6_VARIANTS = {"PZ6", "pz6", "PZ_6", "Pz6", "6", 6}

        for prefix in prefixes_tuple:
            for coll_name in [f"{prefix}ew_cases", f"{prefix}ew_cases_archived"]:
                try:
                    docs = list(db.collection(coll_name).where("status", "==", "zakonczony").limit(5000).stream())
                    diag["by_collection"][coll_name] = len(docs)
                    diag["total_zakonczony"] += len(docs)

                    for d in docs:
                        data = d.to_dict() or {}
                        rp = data.get("result_pz")
                        if rp is not None and str(rp).strip() != "":
                            diag["with_result_pz"] += 1
                            diag["result_pz_values"][str(rp)] += 1

                        ca = data.get("completed_at")
                        if ca:
                            diag["with_completed_at"] += 1
                            try:
                                if hasattr(ca, "astimezone"):
                                    cd_str_diag = ca.astimezone(tz_pl).strftime("%Y-%m-%d")
                                else:
                                    cd_str_diag = str(ca)[:10]
                                diag["completed_at_dates_yyyy_mm_dd"][cd_str_diag] += 1
                            except Exception:
                                pass

                        # Filtr PZ6 (toleruje warianty)
                        if rp not in PZ6_VARIANTS:
                            continue
                        ...
                        try:
                            if hasattr(ca, "astimezone"):
                                cd_str = ca.astimezone(tz_pl).strftime("%Y-%m-%d")
                            else:
                                cd_str = str(ca)[:10]
                        except Exception:
                            continue
                        ...
                        rows.append({
                            "doc_id": d.id,
                            "coll": coll_name,
                            "numer_zamowienia": str(data.get("numer_zamowienia", "")).strip(),
                            "assigned_to": data.get("assigned_to"),
                            "autopilot_assigned_to": data.get("autopilot_assigned_to"),
                            "skipped_by": data.get("skipped_by"),
                    ...
                    st.caption(f"⚠️ {coll_name}: {e}")
                    diag["by_collection"][coll_name] = f"ERROR: {e}"
        return rows, diag

    raw_rows, diag = fetch_pz6_cases_for_date(t5_date_str, tuple(active_prefixes))

    # === Dedup po numer_zamowienia (najnowszy completed_at wygrywa) ===
    by_nr = {}
    no_nr_count = 0
    for r in raw_rows:
        nr = r["numer_zamowienia"]
        if not nr:
            no_nr_count += 1
            continue
        ...
        if not existing:
            ...
            # Porównaj completed_at — zostaw nowszy
            try:
                if r["completed_at"] > existing["completed_at"]:
                    by_nr[nr] = r
            except Exception:
                pass
    cases = list(by_nr.values())

    if not cases:
        st.warning(
            f"🔍 Brak case'ów z `result_pz=PZ6` i `completed_at` z dnia **{t5_date_str}** "
            f"w środowisku `{', '.join(p or 'prod' for p in active_prefixes)}`."
        )
        st.caption(
            f"Surowe case'y znalezione (przed filtrami): {len(raw_rows)}. "
            f"Bez numeru zamówienia: {no_nr_count}."
        )
        # === DIAGNOSTYKA — co właściwie jest w bazie ===
        st.markdown("---")
        st.markdown("### 🔍 Diagnostyka — co jest w bazie")
        st.markdown(f"**Skanowano case'ów ze `status=zakonczony`:** {diag['total_zakonczony']}")
        st.markdown(f"**Z polem `result_pz` (jakimkolwiek):** {diag['with_result_pz']}")
        st.markdown(f"**Z polem `completed_at`:** {diag['with_completed_at']}")

        st.markdown("**Per kolekcja:**")
        for coll, n in diag["by_collection"].items():
            st.markdown(f"- `{coll}`: {n}")

        if diag["result_pz_values"]:
            st.markdown("**Unikalne wartości `result_pz` w zakończonych case'ach:**")
            for val, n in diag["result_pz_values"].most_common(20):
                st.markdown(f"- `{val}` → {n} przypadków")
        else:
            st.markdown("⚠️ **Żaden zakończony case nie ma pola `result_pz`** — apka nie ustawia go (sprawdź `ew_complete_case`)")

        if diag["completed_at_dates_yyyy_mm_dd"]:
            st.markdown("**Top 15 dat `completed_at` (yyyy-mm-dd) w bazie:**")
            for date, n in diag["completed_at_dates_yyyy_mm_dd"].most_common(15):
                marker = " ⬅️ wybrany dzień" if date == t5_date_str else ""
                st.markdown(f"- `{date}`: {n}{marker}")

        st.stop()

    # === Operator: assigned_to → fallback autopilot_assigned_to → '?' ===
    def _operator(r):
        op = r.get("assigned_to")
        if op and str(op).strip() and str(op).strip().lower() != "none":
            return str(op).strip()
        ap = r.get("autopilot_assigned_to")
        if ap and str(ap).strip() and str(ap).strip().lower() != "none":
            return str(ap).strip()
        cb = r.get("completed_by")
        if cb and str(cb).strip() and str(cb).strip().lower() != "none":
            return str(cb).strip()
        return "?"

    for r in cases:
        r["operator"] = _operator(r)

    # === Typ towaru z cache ===
    typ_cache5, _, _ = fetch_typ_cache()

    def _norm_kat(nr):
        info = typ_cache5.get(str(nr).strip())
        if not info:
            return "Nieprzypisane"
        kat = info.get("kategoria", "Brak danych")
        if kat == "Kolektor":
            return "Kolektor"
        if kat == "Skrzynia biegów":
            return "Skrzynia biegów"
    ...
    for r in cases:
        r["kategoria"] = _norm_kat(r["numer_zamowienia"])
    ...
    if t5_typ != "Wszystkie":
        target = TYP_MAP_T5[t5_typ]
        cases = [r for r in cases if r["kategoria"] == target]

    # === Metryki ===
    st.markdown("---")
    total = len(cases)
    unique_ops = len({r["operator"] for r in cases})
    n_kolektor = sum(1 for r in cases if r["kategoria"] == "Kolektor")
    n_skrzynia = sum(1 for r in cases if r["kategoria"] == "Skrzynia biegów")
    n_brak = sum(1 for r in cases if r["kategoria"] == "Nieprzypisane")

    mcol1, mcol2, mcol3, mcol4, mcol5 = st.columns(5)
    mcol1.metric("💎 Diamenty", total)
    mcol2.metric("👥 Operatorów", unique_ops)
    mcol3.metric("🔩 Kolektor", n_kolektor)
    mcol4.metric("🔧 Skrzynia", n_skrzynia)
    mcol5.metric("❓ Nieprzypisane", n_brak)

    # === Wykres per operator ===
    st.markdown("---")
    st.markdown("### 📊 Diamenty per operator")
    op_counts = {}
    for r in cases:
        op_counts[r["operator"]] = op_counts.get(r["operator"], 0) + 1
    df_op = pd.DataFrame(
        [{"Operator": k, "Diamenty": v} for k, v in op_counts.items()]
    ).sort_values("Diamenty", ascending=False)
    st.bar_chart(df_op.set_index("Operator"), height=380)

    # === Sekcja: diff z forum ===
    st.markdown("---")
    st.markdown("### 🔍 Zamy dyskusyjne — diff z forum (źródło prawdy)")
    st.caption(
        "Wklej listę numerów z forum (jeden per linia lub przecinkami). Monitor pokaże 3 sekcje: "
        "**tylko forum** (np. AutoSzturchacz lub case zlecony poza panelem), "
        "**tylko apka** (operator kliknął PZ6 ale nie ma na forum), **w obu** (potwierdzone)."
    )
    forum_input = st.text_area(
        "📋 Numery z forum:",
        height=140,
        key="_t5_forum_input",
        placeholder="368247\n377312\n378054\n...",
    ...
    if forum_input.strip():
        import re as _re_t5
        forum_nrs_raw = _re_t5.split(r"[\n,;\s]+", forum_input.strip())
        forum_nrs = sorted({s.strip() for s in forum_nrs_raw
                            if s.strip().isdigit() and 5 <= len(s.strip()) <= 7})
        apka_nrs = sorted({str(r["numer_zamowienia"]).strip() for r in cases})

        forum_set = set(forum_nrs)
        apka_set = set(apka_nrs)

        only_forum = sorted(forum_set - apka_set, key=lambda x: int(x))
        only_apka = sorted(apka_set - forum_set, key=lambda x: int(x))
        both = sorted(forum_set & apka_set, key=lambda x: int(x))

        st.markdown(
            f"**📋 Wynik:** forum **{len(forum_nrs)}** | apka **{len(apka_nrs)}** | match **{len(both)}** | "
            f"różnice: tylko forum **{len(only_forum)}** + tylko apka **{len(only_apka)}**"
        )

        col_a, col_b, col_c = st.columns(3)
        with col_a:
            st.markdown(f"#### ⚠️ Tylko forum ({len(only_forum)})")
            st.caption("Najpewniej AutoSzturchacz albo zlecone poza panelem apki.")
            if only_forum:
                st.code("\n".join(only_forum), language="text")
            else:
                st.success("✅ Wszystko z forum jest w apce.")
        with col_b:
            st.markdown(f"#### 🤖 Tylko apka ({len(only_apka)})")
            st.caption("Operator kliknął PZ6 w panelu, ale na forum brak. Zapomnienie?")
            if only_apka:
                op_for_nr = {str(r["numer_zamowienia"]).strip(): r["operator"] for r in cases}
                lines = [f"{nr} → {op_for_nr.get(nr, '?')}" for nr in only_apka]
                st.code("\n".join(lines), language="text")
            else:
                st.success("✅ Wszystko z apki jest na forum.")
        with col_c:
            st.markdown(f"#### ✅ W obu ({len(both)})")
            st.caption("Prawdziwe diamenty potwierdzone w dwóch źródłach.")
            if both:
                with st.expander(f"Zobacz {len(both)} numerów"):
                    st.code("\n".join(both), language="text")

    # === Tabela ===
    st.markdown("---")
    st.markdown("### 📋 Pełna lista diamentów (z `ew_cases.result_pz`)")
    df_full = pd.DataFrame([
        {
            "Numer zamówienia": r["numer_zamowienia"],
            "Operator": r["operator"],
            "Typ towaru": r["kategoria"],
            "Grupa": r.get("grupa") or "?",
            "Result tag": r.get("result_tag") or "?",
            "Czas zakończenia": r["completed_at"].astimezone(tz_pl).strftime("%H:%M:%S") if r.get("completed_at") and hasattr(r["completed_at"], "astimezone") else "?",
            "Kolekcja": r.get("coll", "?"),
            "Środowisko": r.get("env", "?"),
        }
        for r in cases
    ]).sort_values(by=["Operator", "Numer zamówienia"])
    st.dataframe(df_full, use_container_width=True, hide_index=True)


# ============================================================
# ====== ZAKŁADKA 6: DIAMENTY (forum / ew_diamond_log) =======
# ============================================================
with tab6:
    st.header("💎 Diamenty (forum) — `ew_diamond_log`")
    st.caption(
        "Źródło prawdy: **moment wysłania zlecenia kuriera na forum** (cel AUTOS_KURIERZY, PZ6, bump=0). "
        "Operator i czatoszturek (AutoSzturchacz) zapisują 1 diament przy każdym takim wpisie. "
        "Append-only, dedup po numerze/dzień. Bumpy, eskalacje i fazy PZ7+ NIE liczone. "
        "Działa od wdrożenia patcha apki — wcześniejsze dni będą puste."
    )

    # ===== Filtry =====
    fc1, fc2 = st.columns([3, 2])
    with fc1:
        t6_range = st.date_input(
            "📅 Zakres dat:",
            value=(today.date() - timedelta(days=6), today.date()),
            key="_t6f_range",
        )
    with fc2:
        t6_src = st.radio(
            ...
            index=0, horizontal=True, key="_t6f_src",
        )
    ...
    with fc3:
        t6_kat = st.radio(
            ...
            index=0, horizontal=True, key="_t6f_kat",
        )
    with fc4:
        t6_grupa = st.selectbox(
            "🌍 Grupa:",
            options=["Wszystkie", "DE", "FR", "UKPL"],
            index=0, key="_t6f_grupa",
        )

    # Normalizuj zakres dat
    if isinstance(t6_range, (tuple, list)):
        if len(t6_range) == 2:
            t6_from, t6_to = t6_range
        elif len(t6_range) == 1:
            t6_from = t6_to = t6_range[0]
        else:
            t6_from = t6_to = today.date()
    else:
        t6_from = t6_to = t6_range

    # ===== Pobierz z ew_diamond_log =====
    @st.cache_data(ttl=60)
    def fetch_diamond_log_t6(date_from_iso, date_to_iso, prefixes_tuple):
        """Czyta {prefix}ew_diamond_log/{date}/numbers/* dzień po dniu.
        Dedup po (date_str, numer) — przy nakładających się prefixach pierwszy wygrywa."""
        ...
        while cur <= d_to:
            ...
            for prefix in prefixes_tuple:
                try:
                    ...
                    for doc in docs:
                        data = doc.to_dict() or {}
                        nr = str(data.get("numer_zamowienia", doc.id)).strip()
                        key = (ds, nr)
                        if key in seen:
                            continue
                        seen.add(key)
                        rows.append({
                            ...
                            "kurier_opcja": data.get("kurier_opcja"),
    ...
    diamonds = fetch_diamond_log_t6(t6_from.strftime("%Y-%m-%d"), t6_to.strftime("%Y-%m-%d"), tuple(active_prefixes))

    if not diamonds:
        st.warning(f"🔍 Brak diamentów w `ew_diamond_log` dla **{t6_from} → {t6_to}** ({prefixes_label}).")
        st.info(
            "ℹ️ Możliwe: patch apki (`forum_module` z `log_diamond`) jeszcze nie wdrożony · "
            "nikt nie zlecił kuriera w tym zakresie · zakres sprzed wdrożenia. "
            f"Sprawdź czy apka pisze do `{(active_prefixes[0] if active_prefixes else '')}ew_diamond_log`."
        )
        st.stop()

    # ===== Kategoria: typ_towaru_cache (po numerze) → resolved_index ORG/REG/BMW=kolektor → kategoria_towaru z logu =====
    typ_cache6, _, _ = fetch_typ_cache()

    def _kat_t6(nr, kat_log):
        info = typ_cache6.get(str(nr).strip())
        if info:
            kat = info.get("kategoria")
            if kat in ("Kolektor", "Skrzynia biegów"):
                return kat
            # fallback: resolved_index zaczyna się ORG/REG/BMW → kolektor
            idx = str(info.get("resolved_index") or "").upper().strip()
            if idx[:3] in ("ORG", "REG", "BMW"):
                return "Kolektor"
        # fallback: kategoria_towaru z logu (z TOWAR_TYP w momencie zlecenia)
        if kat_log:
    ...
    for r in diamonds:
        r["_kat"] = _kat_t6(r["numer_zamowienia"], r.get("kategoria_towaru"))
        stype = str(r.get("source_type", "operator")).lower()
        r["_is_auto"] = ("auto" in stype or "szturch" in stype or "bot" in stype)

    # ===== Filtry =====
    if t6_src == "🧑 Operatorzy":
        diamonds = [r for r in diamonds if not r["_is_auto"]]
    elif t6_src == "🤖 Czatoszturek":
        diamonds = [r for r in diamonds if r["_is_auto"]]

    if t6_kat == "🔩 Kolektor":
        diamonds = [r for r in diamonds if r["_kat"] == "Kolektor"]
    elif t6_kat == "🔧 Skrzynia":
        diamonds = [r for r in diamonds if r["_kat"] == "Skrzynia biegów"]

    if t6_grupa != "Wszystkie":
        diamonds = [r for r in diamonds if str(r.get("grupa") or "").upper() == t6_grupa]

    # Filtr operatora (opcjonalny — lista z danych po filtrach źródła/grupy)
    ops_available = sorted({r["operator"] for r in diamonds})
    with fc5:
        t6_op = st.selectbox("🧑 Operator:", options=["Wszyscy"] + ops_available, index=0, key="_t6f_op")
    if t6_op != "Wszyscy":
        diamonds = [r for r in diamonds if r["operator"] == t6_op]
    ...
    st.markdown("---")
    total = len(diamonds)
    n_oper = sum(1 for r in diamonds if not r["_is_auto"])
    n_auto = sum(1 for r in diamonds if r["_is_auto"])
    n_kol = sum(1 for r in diamonds if r["_kat"] == "Kolektor")
    n_skrz = sum(1 for r in diamonds if r["_kat"] == "Skrzynia biegów")

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("💎 Diamenty", total)
    m2.metric("🧑 Operatorzy", n_oper)
    m3.metric("🤖 Czatoszturek", n_auto)
    m4.metric("🔩 Kolektor", n_kol)
    m5.metric("🔧 Skrzynia", n_skrz)

    # ===== Wykres stacked per dzień (operator vs czatoszturek) =====
    st.markdown("---")
    ...
    with cc1:
        ...
        st.dataframe(pd.DataFrame([
            {"Źródło": "🧑 Operatorzy", "Ilość": n_oper},
            {"Źródło": "🤖 Czatoszturek", "Ilość": n_auto},
        ]), use_container_width=True, hide_index=True)
    with cc2:
        st.markdown("##### 🏷️ Kategoria")
        by_kat = {}
        for r in diamonds:
            by_kat[r["_kat"]] = by_kat.get(r["_kat"], 0) + 1
        st.dataframe(pd.DataFrame([{"Kategoria": k, "Ilość": v} for k, v in by_kat.items()]).sort_values("Ilość", ascending=False),
                     use_container_width=True, hide_index=True)
    with cc3:
        st.markdown("##### 🌍 Grupa")
        by_g = {}
        ...
        st.dataframe(pd.DataFrame([{"Grupa": k, "Ilość": v} for k, v in by_g.items()]).sort_values("Ilość", ascending=False),
                     use_container_width=True, hide_index=True)
    ...
    df_full = pd.DataFrame([
        {
            ...
            "Operator": r["operator"],
            "Źródło": "🤖 Czatoszturek" if r["_is_auto"] else "🧑 Operator",
            "Kategoria": r["_kat"],
            "Przewoźnik": str(r.get("kurier") or "?").upper(),
            "Opcja": str(r.get("kurier_opcja") or "?"),
