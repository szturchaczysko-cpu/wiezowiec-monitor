                ...
                with st.expander("🤖 Odśwież cache w agencie (skopiuj prompt → wklej w Dashboard DSG)", expanded=(n_no_data > 0 and cache_n == 0)):
                    ...
                    if refresh_mode.startswith("Tylko"):
                        ...
                        if not missing_nrs:
                            ...
                            nr_list = ", ".join(str(n) for n in missing_nrs)
                            agent_prompt = f"""Zbuduj mapowanie numer_zamowienia → typ towaru dla wskazanych zamówień i ZWRÓĆ wynik jako JSON do skopiowania.

LISTA NUMERÓW ZAMÓWIEŃ ({len(missing_nrs)} szt.):
{nr_list}

DLA KAŻDEGO numeru wykonaj:

1. Spróbuj odczytać dokument w `test_ew_cases_archived`, `test_ew_cases`, `ew_cases_archived` lub `ew_cases` po polu `numer_zamowienia`. Wyciągnij `index_handlowy`.

2. Rozwiąż indeks:
   - jeśli `index_handlowy` ≠ "" → użyj go (source='index_handlowy')
   - inaczej SQL (MSSQL STEEPC): SELECT TOP 1 zksIndex_WEB FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr = <numer> AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '' ORDER BY zksCenaNetto DESC → użyj zksIndex_WEB (source='sql')
   - jeśli pusto → kategoria='Nieznany', tartNazwa=null, tartID=null, source='lookup_failed'

3. Resolve typ (Postgres maggo): SELECT ta."tartID", ta."tartNazwa" FROM "public"."wmsArtykuly" a JOIN "public"."wmsTypArtykulu" ta ON ta."tartID" = a."artTyp" WHERE a."artIndeks" = <indeks>

4. Mapuj kategorię biznesową:
   - tartID ∈ (4, 10, 15) → 'Kolektor'
   - tartID ∈ (3, 5, 6, 7, 9, 13, 14, 16) → 'Skrzynia biegów'
   - reszta → 'Inne'

5. **NIE zapisuj do Firestore** — zamiast tego zbierz wyniki w pamięci.

WAŻNE — OPTYMALIZACJA: zamiast pętli po 1813 numerach, zrób tak:
- a) Pobierz JEDNYM zapytaniem WSZYSTKIE wpisy w MSSQL: SELECT zksZknZamNr, zksIndex_WEB FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr IN (<lista>) AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '' ORDER BY zksZknZamNr, zksCenaNetto DESC. Z tego bierz pierwszy indeks per numer.
- b) Z otrzymanych indeksów zrób JEDNO zapytanie do Postgres: SELECT a."artIndeks", ta."tartID", ta."tartNazwa" FROM "public"."wmsArtykuly" a JOIN "public"."wmsTypArtykulu" ta ON ta."tartID" = a."artTyp" WHERE a."artIndeks" IN (<lista>).
- c) Połącz wyniki w Pythonie.

NA KOŃCU zwróć JSON w czystym bloku ```json (BEZ KOMENTARZY, gotowy do parsowania):

```json
{{
  "360603": {{"kategoria": "Kolektor", "tartNazwa": "Kolektor", "tartID": 4, "resolved_index": "ORG30_BMW_SAKS_PRZE_KPL", "source": "index_handlowy"}},
  "362815": {{"kategoria": "Skrzynia biegów", "tartNazwa": "Skrzynia", "tartID": 3, "resolved_index": "20620MBGRUP10", "source": "sql"}},
  "356287": {{"kategoria": "Skrzynia biegów", "tartNazwa": "Skrzynia", "tartID": 3, "resolved_index": "256222S", "source": "sql"}}
}}
```

Dodatkowo wypisz podsumowanie tekstowe: ile Kolektor / Skrzynia / Inne / Nieznany."""
                    else:
