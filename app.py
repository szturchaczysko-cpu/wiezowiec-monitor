            ...
            if dd_days_n < 1:
                ...
                with st.expander("🤖 Odśwież cache w agencie (skopiuj prompt → wklej w Dashboard DSG)", expanded=(n_no_data > 0 and cache_n == 0)):
                    ...
                    if refresh_mode.startswith("Tylko"):
                        # Zbuduj listę brakujących numerów zamówień
                        missing_nrs = sorted({c["numer_zamowienia"] for c in pz6_filtered
                                              if c["towar_typ_source"] == "brak"
                                              and c["numer_zamowienia"] and c["numer_zamowienia"] != "?"})
                        if not missing_nrs:
                            st.success("✅ Brak case'ów do uzupełnienia — wszystkie mają już typ z cache lub regexa.")
                            agent_prompt = ""
                        else:
                            nr_list = ", ".join(str(n) for n in missing_nrs)
                            agent_prompt = f"""Zbuduj cache typu towaru dla wskazanych zamówień Firebase → zapisz do kolekcji `typ_towaru_cache`.

LISTA NUMERÓW ZAMÓWIEŃ DO UZUPEŁNIENIA ({len(missing_nrs)} szt.):
{nr_list}

DLA KAŻDEGO numer_zamowienia z listy wykonaj:

1. Spróbuj odczytać dokument `test_ew_cases_archived/...` lub `test_ew_cases/...` LUB prod-analogi, znajdując case po polu `numer_zamowienia`. Wyciągnij `index_handlowy`.

2. Rozwiąż indeks:
   - jeśli `index_handlowy` ≠ "" → użyj go
   - inaczej SQL (MSSQL STEEPC): SELECT TOP 1 zksIndex_WEB FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr = <numer> AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '' ORDER BY zksCenaNetto DESC
   - jeśli pusto → zapisz cache z kategoria='Nieznany', source='lookup_failed' i przejdź dalej
...
4. Mapuj na kategorię biznesową:
   - tartID ∈ (4, 10, 15) → 'Kolektor'
   - tartID ∈ (3, 5, 6, 7, 9, 13, 14, 16) → 'Skrzynia biegów'
   - reszta (1, 2, 8, 11, 12, 17) → 'Inne'

5. Zapisz do Firestore `typ_towaru_cache/{{numer_zamowienia}}`: numer_zamowienia, resolved_index, tartID, tartNazwa, kategoria, source ('index_handlowy' | 'sql' | 'lookup_failed'), updated_at=SERVER_TIMESTAMP. Użyj merge=True.

Po zakończeniu zwróć podsumowanie: ile case'ów ✅ Kolektor, ile ⚙️ Skrzynia, ile 📦 Inne, ile ❓ lookup_failed (i pierwsze 10 numerów które się nie udały)."""
                    else:
                        agent_prompt = """Zbuduj/odśwież PEŁNY cache typu towaru w Firestore → kolekcja `typ_towaru_cache`.

Dla KAŻDEGO dokumentu w:
- `test_ew_cases_archived`
- `test_ew_cases`
- `ew_cases_archived` (jeśli masz read do prod)
- `ew_cases`

wykonaj:

1. Wczytaj `numer_zamowienia`, `index_handlowy`. (Jeśli już istnieje wpis w `typ_towaru_cache/{numer_zamowienia}` i nie jest 'lookup_failed' → pomiń, oszczędzaj czas.)

2. Rozwiąż indeks:
   - jeśli `index_handlowy` ≠ "" → użyj go
   - inaczej SQL (MSSQL STEEPC): SELECT TOP 1 zksIndex_WEB FROM [STEEPC].[dbo].[scZAMKLISZCZEG] WHERE zksZknZamNr = <numer> AND zksCenaNetto > 0 AND zksIndex_WEB IS NOT NULL AND zksIndex_WEB <> '' ORDER BY zksCenaNetto DESC
   - jeśli pusto → kategoria='Nieznany', source='lookup_failed'
...
5. Zapis: `typ_towaru_cache/{numer_zamowienia}` z polami numer_zamowienia, resolved_index, tartID, tartNazwa, kategoria, source, updated_at=SERVER_TIMESTAMP, merge=True.

Po zakończeniu zwróć:
- ile case'ów przetworzono
- ile w każdej kategorii
- ile lookup_failed + pierwsze 10 numerów do diagnozy"""
